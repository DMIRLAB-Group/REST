...
from models.MultiVAE import MultiVAE_rate
...
def train(model, opt, loader, args):
    ...
    for batch in loader:
        ...
        opt.zero_grad()
        pred_rate, mu, logvar = model(parsed_batch)
        ...
        loss += args.beta * -0.5 * (1 + logvar - mu.pow(2) - logvar.exp()).sum(dim=-1).mean()
        loss.backward()
        opt.step()
...
def evaluate(model, eval_loader, args):
    ...
    with torch.no_grad():
        for batch in eval_loader:
            parsed_batch = parse_batch(batch)
            pred_rate, _, _ = model(parsed_batch)
            true_rate = parsed_batch[-1]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default='MultiVAE')
    ...
    for r in range(1, args.repeat + 1):
        ...
        model = MultiVAE_rate(user_num, item_num, args).to(device)
