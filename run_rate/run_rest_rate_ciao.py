...
def train(model, opt, lr_shdr, loader, args):
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        # emb_reg = user_emb.norm(dim=-1).pow(2).mean() + \
        #           item_emb.norm(dim=-1).pow(2).mean() + \
        #           unexp_item_emb.norm(dim=-1).pow(2).mean()
        reg_term = (torch.norm(user_emb) ** 2 +
                    torch.norm(item_emb) ** 2 +
                    torch.norm(unexp_item_emb) ** 2) / 2
        emb_reg = args.emb_l2rg * reg_term / user_emb.shape[0]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--alpha', type=float, default=0.1, help='P(G|U) weight')
    parser.add_argument('--beta', type=float, default=0.0, help='KL weight')
    parser.add_argument('--w_unexp', type=float, default=1.0, help='unexp loss weight')
