...
from baselines.LightGCN import LightGCN_rate

from utils.data_loader import load_ds_baseline_rate, get_ui_graph
...
def train(model, opt, loader, args):
    ...
    for batch in loader:
        ...
        opt.zero_grad()
        pred_rate, users_emb, items_emb = model(parsed_batch)
        true_rate = parsed_batch[-1]

        if args.rescale_rate:
            pred_rate = torch.sigmoid(pred_rate)
            true_rate = (true_rate - 1) / 4.0

        loss = F.mse_loss(pred_rate, true_rate)
        loss += args.l2rg * 0.5 * (users_emb.norm() + items_emb.norm())
        loss.backward()
        opt.step()
...
def evaluate(model, eval_loader, args):
    ...
    with torch.no_grad():
        ...
        for batch in eval_loader:
            parsed_batch = parse_batch(batch)
            pred_rate = model.eval_forward(all_users, all_items, parsed_batch)
            true_rate = parsed_batch[-1]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--l2rg', type=float, default=1e-8)
    ...
    ui_graph = get_ui_graph(args.dataset, train_u_ir, user_num, item_num)
    ...
    for r in range(1, args.repeat + 1):
        ...
        model = LightGCN_rate(3, user_num, item_num, ui_graph, args).to(device)
        opt = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.l2rg)
        patience = 0
        best_mae = best_rmse = 1e6

        for epoch in range(2, args.max_epochs):
