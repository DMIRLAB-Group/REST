...
from baselines.BPRMF import BPRMF_rate
...
def train(model, opt, loader, args):
    ...
    for batch in loader:
        ...
        opt.zero_grad()
        pos_logits, neg_logits, user_emb, item_emb, neg_item_emb = model.cvib_forward(parsed_batch)
        ...
        emb_loss = args.l2rg * (user_emb.pow(2).sum(dim=-1).mean() +
                                item_emb.pow(2).sum(dim=-1).mean() +
                                neg_item_emb.pow(2).sum(dim=-1).mean())

        loss = label_loss + info_loss + emb_loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default='CVIB-MF')
    ...
    for r in range(1, args.repeat + 1):
        ...
        model = BPRMF_rate(user_num, item_num, args).to(device)
