...
class TrustMF(nn.Module):
    def __init__(self, user_num, item_num, args):
        super(TrustMF, self).__init__()
        self.user_num = user_num
        self.item_num = item_num
        self.dev = torch.device(args.device)
        self.user_embs = nn.Embedding(user_num, args.edim, padding_idx=0)
        self.user_context_embs = nn.Embedding(user_num, args.edim, padding_idx=0)
        ...
        nn.init.uniform_(self.user_context_embs.weight, a=-0.5 / user_num, b=0.5 / user_num)
    ...
    def forward(self, batch):
        ...
        nbrs_emb = self.user_context_embs(nbr)
...
class TrustMF_rate(nn.Module):
    def __init__(self, user_num, item_num, args):
        super(TrustMF_rate, self).__init__()
        self.user_num = user_num
        self.item_num = item_num
        self.dev = torch.device(args.device)
        self.user_embs = nn.Embedding(user_num, args.edim, padding_idx=0)
