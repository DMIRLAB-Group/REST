...
class BPRMF_rank(nn.Module):
    def __init__(self, user_num, item_num, args):
        super(BPRMF_rank, self).__init__()
...
class BPRMF_rate(nn.Module):
    def __init__(self, user_num, item_num, args):
        super(BPRMF_rate, self).__init__()
        self.user_num = user_num
        self.item_num = item_num
        self.dev = torch.device(args.device)
        self.user_embs = nn.Embedding(user_num, args.edim, padding_idx=0)
    ...
    def forward(self, batch):
        ...
        return pred_rate, user_emb, item_emb
    ...
    def cvib_forward(self, batch):
        ...
        user_emb = self.user_embs(user)
        item_emb = self.item_embs(item)
        neg_item_emb = self.item_embs(neg_item)
        pred_rate = self.pred(user_emb, item_emb)
        neg_pred_rate = self.pred(user_emb, neg_item_emb)
        return pred_rate, neg_pred_rate, user_emb, item_emb, neg_item_emb
